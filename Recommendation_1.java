...
public class Recommendation_1 {
	...
	public void generateRecommendations() {
		int[][] preference = readFile(PREFROWCOUNT,COLUMNCOUNT, "d:/u2.base");
//		wirteFile(preference, "d:/pre.txt");
		double[][] pre_minus_mean = minus_mean(preference);
		wirteFile(pre_minus_mean, "d:/pre_mean.txt");
...
//		int[][] test = readFile(TESTROWCOUNT, "u1.test");
		...
		wirteFile(similarityMatrix, "d:/sMatrix.txt");
//		int[][] test = readFile(TESTROWCOUNT, "u1.test");
		...
		File file = new File("d:/result_all.txt"); // 存放数组数据的文件
		FileWriter out = null;
		try {
			...
			for (int j = 0; j < COLUMNCOUNT; j++) {
				if (preference[i][j] == 0) {
					...
					for (int m = 0; m < neighborSerial.size(); m++) {
						sum += similarityMatrix[j][neighborSerial.get(m)]
								* preference[i][neighborSerial.get(m)];
						similaritySum += similarityMatrix[j][neighborSerial
								.get(m)];
					}
					if (sum !=0) {
		...
		for (int i = 0; i < COLUMNCOUNT; i++) {
			for (int j = 0; j < COLUMNCOUNT; j++) {
				if (i == j) {
					...
					similarityMatrix[i][j] = similarityMatrix[j][i] = computeSimilarity(
							item1, item2);
...
//		System.out.println(list1+"  "+list2);
	 ...
	 * @return
	 */
	private int[][] readFile(int rowCount,int columnCount,String fileName) {
	...
	/**
	 * This method is used to compute user mean score by Guanjun Liu uestc
	 * 
	 * @param
	 * @return
	 */
	private double user_means(int[] a) {
		...
		double means = (double)sum / (double)count;
...
//				+ sum_pro);
	...
	public static void main(String[] args) {
		Recommendation_1 application = new Recommendation_1();
